...
export default function DoctorSearchPage() {
    ...
    useEffect(() => {
        ...
        if (specialityFromUrl) {
            setSelectedSpecialty(specialityFromUrl)
    ...
    const fetchDoctors = useCallback(async () => {
        try {
            ...
            const doctorsData = await getFilteredDoctors({
                state,
                district,
                specialty
            });
    ...
    }, [userCoords, locationAvailable, state, district, specialty])
    ...
    return (
        <div className="container mx-auto px-4 py-8 bg-gradient-to-br from-emerald-50 to-green-50 min-h-screen">
            <div className="flex items-center gap-4 mb-8">
                <Link href="/" className="flex items-center text-green-700 hover:text-emerald-800">
                    <ArrowLeft className="w-5 h-5 mr-2"/>
                    Back to Home
                </Link>
            </div>

            {/* Search Bar - Repositioned to top right */}
                ...
                <form onSubmit={handleSearch} className="flex gap-4 w-full max-w-md">
                    <div className="relative flex-1">
                        <input
                            ...
                            placeholder="Search freelancers by name or service..."
                            className="w-full px-4 py-3 pl-12 rounded-lg border-2 border-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                        />
                    ...
                    <button
                        type="submit"
                        className="px-6 py-3 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-lg hover:from-emerald-700 hover:to-green-700 transition-all duration-300"
                    >
                        Search
                    </button>
                </form>
            ...
            {/* Specialty Slider - Moved to bottom */}
            {error && (
            ...
            {!locationAvailable && (
                <div className="bg-green-100 text-green-800 p-3 mb-4 rounded">
                    <p>⚠️ Location not available - showing all freelancers</p>
                </div>
            )}
            ...
            {loading ? (
                <div className="flex justify-center items-center h-64">
                    ...
                    <span className="ml-3 text-green-700">Loading freelancers...</span>
                </div>
            ) : error ? (
                ...
                <>
                    <div className="mb-4">
                        {filteredDoctors.length > 0 && (
                            <p className="text-sm text-green-700 font-medium">
                                Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredDoctors.length)} of {filteredDoctors.length} freelancers
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {currentDoctors.length > 0 ? (
                            currentDoctors.map((doctor) => (
                                <div key={doctor.id}
                                     className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 border-2 border-emerald-100 hover:border-emerald-300">
                                    <div className="p-6">
                                            ...
                                            <div className="flex-shrink-0">
                                                {doctor.imageUrl ? (
                                                    <Image
                                                        ...
                                                        width={80}
                                                        height={80}
                                                        className="rounded-full object-cover border-3 border-emerald-200"
                                                    />
                                                ) : (
                                                    <div
                                                        className="w-20 h-20 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-xl">
                                                        {doctor.fullName.charAt(0)}
                                                    </div>
                                                )}
                                            </div>
                                            <div>
                                                <h3 className="text-lg font-bold text-green-800">{doctor.fullName}</h3>
                                                ...
                                                {doctor.distance && (
                                                    <div className="flex items-center mt-1">
                                                        ...
                                                        <span className="text-xs text-emerald-600 font-medium">{doctor.distance} away</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4">
                                            ...
                                            <div className="flex justify-between items-center mt-4">
                                                {doctor.consultationFees ? (
                                                    ...
                                                    <span className="text-green-600">Fee not specified</span>
                                                )}

                                                <Link
                                                    href={`/freelancers/${doctor.id}`}
                                                    className="px-6 py-2 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-full hover:from-emerald-700 hover:to-green-700 transition-all duration-300 font-medium"
                                                >
                                                    View Profile
                                                </Link>
                                            </div>
                            ...
                            <div className="col-span-full text-center py-12">
                                <p className="text-green-600 text-lg font-medium">
                                    No freelancers found {searchQuery ? `matching "${searchQuery}"` : ''}
                                    {selectedSpecialty ? ` in ${selectedSpecialty}` : ''}
                                    {state ? ` in ${state}` : ''}
                                    {district ? `, ${district}` : ''}
                                </p>
                    ...
                    <div className="mt-12 mb-8">
                        <h3 className="text-xl font-bold text-green-800 mb-6 text-center">Filter by Service</h3>
                            ...
                            <div className="flex space-x-4 px-4 justify-center">
                                ...
                                >
                                    <Button
                                        ...
                                        className={`flex flex-col items-center justify-center px-6 py-4 rounded-xl min-w-[120px] ${
                                            !selectedSpecialty
                                                ? 'bg-emerald-200 border-2 border-emerald-600 text-emerald-800'
                                ...
                                {specialties.map((specialty) => (
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="w-16 h-16 rounded-full overflow-hidden mb-3 relative border-2 border-emerald-200">
                                                {specialty.imageUrl ? (
                                                    <Image
                    ...
                    {filteredDoctors.length > itemsPerPage && (
                        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-8">
                            <div className="flex items-center space-x-2">
                                <span className="text-sm text-green-700 font-medium">Items per page:</span>
                                <Select
                                ...
                                >
                                    <SelectTrigger className="w-20 bg-white border-2 border-emerald-300 text-green-700">
                            ...
                            <div className="flex space-x-2">
                                ...
                                {totalPages > 5 && currentPage < totalPages - 2 && (
                                    <span className="px-3 py-1 flex items-center text-green-700">...</span>
                                )}
